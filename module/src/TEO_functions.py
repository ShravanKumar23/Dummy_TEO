...
def loadData(
    filePath, sheetSets, sheetParams, sheetParamsDefault, sheetMcs, sheetMcsNum
):
    ...
    mcs_df["DEFAULT_SETTING"] = mcs_df["DEFAULT_SETTING"].apply(
...
def createVariable(_name, _v):
    return newVarDict(
        _name, _v[_name]["lb"], _v[_name]["ub"], _v[_name]["cat"], _v[_name]["sets"]
    )
...
def newVarDict(_name, _lb, _ub, _cat, _sets):
    ...
    """
    return {
        ci(v): pulp.LpVariable(f"{_name}_" + ci(v), lowBound=_lb, upBound=_ub, cat=_cat)
...
def saveResultsTemporary(_model, _scenario_i, variables):
    ...
    res = tuple(
        [
            v
            for v in _model.variables()
            if v.name != "Cost" and v.name != "RateOfProductionByTechnologyByMode"
        ]
    )
    other_df = pd.DataFrame(
        data={
            "NAME": [v.name.split("_")[0] for v in res],
    ...
    df["REGION"] = [
        e[i.index("r")] if "r" in i else np.nan
        for i, e in zip(df["INDICES"], df["ELEMENTS"])
    ]
    df["REGION2"] = [
        e[i.index("rr")] if "rr" in i else np.nan
        for i, e in zip(df["INDICES"], df["ELEMENTS"])
    ]
    df["DAYTYPE"] = [
        e[i.index("ld")] if "ld" in i else np.nan
        for i, e in zip(df["INDICES"], df["ELEMENTS"])
    ]
    df["FUEL"] = [
        e[i.index("f")] if "f" in i else np.nan
        for i, e in zip(df["INDICES"], df["ELEMENTS"])
    ]
    df["EMISSION"] = [
        e[i.index("e")] if "e" in i else np.nan
        for i, e in zip(df["INDICES"], df["ELEMENTS"])
    ]
    df["DAILYTIMEBRACKET"] = [
        e[i.index("lh")] if "lh" in i else np.nan
        for i, e in zip(df["INDICES"], df["ELEMENTS"])
    ]
    df["SEASON"] = [
        e[i.index("ls")] if "ls" in i else np.nan
        for i, e in zip(df["INDICES"], df["ELEMENTS"])
    ]
    df["TIMESLICE"] = [
        e[i.index("l")] if "l" in i else np.nan
        for i, e in zip(df["INDICES"], df["ELEMENTS"])
    ]
    df["MODE_OF_OPERATION"] = [
        e[i.index("m")] if "m" in i else np.nan
        for i, e in zip(df["INDICES"], df["ELEMENTS"])
    ]
    df["STORAGE"] = [
        e[i.index("s")] if "s" in i else np.nan
        for i, e in zip(df["INDICES"], df["ELEMENTS"])
    ]
    df["TECHNOLOGY"] = [
        e[i.index("t")] if "t" in i else np.nan
        for i, e in zip(df["INDICES"], df["ELEMENTS"])
    ]
    df["YEAR"] = [
        e[i.index("y")] if "y" in i else np.nan
        for i, e in zip(df["INDICES"], df["ELEMENTS"])
    ]
...
def GIS_ExchangeCapacities(UseByTechnology, ProductionByTechnology):
    ...
    sortedtable = table.reindex(
        table["VALUE"]
        .sort_values(by=table.columns.droplevel(level=0)[0], ascending=False)
        .index
    )
    append_df = sortedtable.head(
        table["VALUE", table.columns.droplevel(level=0)[0]].value_counts()[
            table["VALUE", table.columns.droplevel(level=0)[0]].max()
        ]
    )
    for col_name in table.columns.droplevel(level=0):
        sortedtable = table.reindex(
            table["VALUE"].sort_values(by=col_name, ascending=False).index
        )
        append_df = append_df.append(
            sortedtable.head(
                table["VALUE", col_name].value_counts()[table["VALUE", col_name].max()]
            )
        )
...
def CreateResults(res_df):
    ...
    for key in TEO_Results.keys():
        output[key] = TEO_Results[key].to_dict("records")
