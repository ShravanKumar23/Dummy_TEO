...
#         model += RateOfStorageDischarge.get(ci(rldlhlssy)) <= StorageMaxDischargeRate.get(ci(rldlhlssy[4:6]), dflt.get('StorageMaxDischargeRate')), ""
    ...
    for rs in REGION_STORAGE: 
        ...
        model += DiscountedSalvageValueByStorage.get(ci(rs)) ==  pulp.lpSum([DiscountedSalvageValueStorage.get(ci(rsy)) for y in YEAR]), ""
        # SI5.1_DiscountingCapitalInvestmentbyStorageBusinessModule
        model += DiscountedCapitalInvestmentByStorage.get(ci(rs)) ==  pulp.lpSum([(CapitalInvestmentStorage.get(ci(rsy)) * (1/ ((1+DiscountRateSto.get(ci(rsy[0:2]), dflt.get('DiscountRateSto')))**(int(rsy[2]) - int(min(YEAR)))))) for y in YEAR]), ""
  
    # ====  Technology  ====   
    for rt in REGION_TECHNOLOGY:
        # CC2.1_DiscountingCapitalInvestmentbytechnology
        model += DiscountedCapitalInvestmentByTechnology.get(ci(rt)) ==  pulp.lpSum([(CapitalInvestment.get(ci(rty)) * (1/((1 + DiscountRateTech.get(ci(rty[0:2]), dflt.get('DiscountRateTech'))) ** (int(rty[2]) - int(min(YEAR)))))) for y in YEAR]), ""
        # OC4.1_DiscountedOperatingCostsbytechnologyBusinessModule
        model += DiscountedOperatingCostByTechnology.get(ci(rt)) ==  pulp.lpSum([(OperatingCost.get(ci(rty)) * (1 / ((1 +  DiscountRateTech.get(ci(rty[0:2]), dflt.get('DiscountRateTech'))) ** (int(rty[2]) - int(min(YEAR)) + 0.5)))) for y in YEAR]), ""   
        # SV4.1_DiscountedSalvageValuebytechnologyBusinessModule
        model += DiscountedSalvageValueByTechnology.get(ci(rt)) ==  pulp.lpSum([DiscountedSalvageValue.get(ci(rty)) for y in YEAR]), ""    
